...
function update(secs) {
  ...
  for (var i = 0; i < brickArray.length; i++) {
    if (
      boundBox.x > brickArray[i].x &&
      boundBox.x + boundBox.width < brickArray[i].x + brickArray[i].width &&
      boundBox.y < brickArray[i].y + brickArray[i].height
    ) {
  ...
  for (var i = 0; i < brickArray.length; i++) {
    if (
      boundBox.y > brickArray[i].y &&
      boundBox.y + boundBox.height < brickArray[i].y + brickArray[i].height &&
      boundBox.x < brickArray[i].x + brickArray[i].width
    ) {
      brickArray.splice(i, 1);
      score = score - 1;
...
function render() {
  ...
  ctx.beginPath();
  ctx.arc(
    boundBox.x + boundBox.width / 2,
    boundBox.y + boundBox.height / 2,
    boundBox.height / 2,
    0,
    2 * Math.PI,
    false
